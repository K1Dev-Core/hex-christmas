...
Citizen.CreateThread(function()
	...
	PromptSetControlAction(OpenToggleUI, 0x760A9C6F)
...
function SetEntityHeadingLookAt(ped, target)
	...
	TaskAchieveHeading(ped, yaw * RAD2DEG)
end
...
local function RemoveAllBlips()
	for i, blip in pairs(treeBlips) do
		RemoveBlip(blip)
	end
	treeBlips = {}
end
...
local function CreateTreeBlips()
	RemoveAllBlips() -- ลบ blips เก่าก่อน
	for i, point in ipairs(Config.TreePoints) do
		local treeKey = tostring(i)  -- แปลง i เป็น string เพื่อใช้เป็น key
		print("collectedTrees[" .. treeKey .. "] = " .. tostring(collectedTrees[treeKey])) -- ตรวจสอบค่า

		-- ตรวจสอบว่า tree ที่จุดนี้ถูกเก็บหรือไม่
		if collectedTrees[treeKey] ~= true then
			local blip = Citizen.InvokeNative(0x554D9D53F696D002, 1664425300, point.x, point.y, point.z)
			SetBlipSprite(blip, 150441873)
			Citizen.InvokeNative(0x9CB1A1623062F402, blip, "#" .. i)
			treeBlips[i] = blip
		end
	end
end
...
-- สร้างต้นคริสมาส
local function CreateChristmasTree(x, y, z)
	local randomModel = "CollectibleModel"

	-- โหลดโมเดล
	LoadModel(randomModel)

	local tree = CreateObject(GetHashKey(randomModel), x, y, z - 2.5, true, true, true)
	SetEntityAsMissionEntity(tree, true, true)
	SetEntityVisible(tree, true)
	FreezeEntityPosition(tree, true)
	SetPickupLight(tree, true)
	return tree
end

-- แอนิเมชันต้นไม้โผล่
local function RaiseTreeAnimation(tree, finalZ)
	local startZ = finalZ - 3.0
	local currentZ = startZ
	local step = 0.008
	local rotationSpeed = 1.5    -- ความเร็วในการหมุน (ปรับได้ตามต้องการ)
	local currentRotationZ = 0.0 -- การหมุนเริ่มต้นที่แกน Z

	while currentZ < finalZ do
		currentZ = currentZ + step
		currentRotationZ = currentRotationZ + rotationSpeed -- เพิ่มค่าการหมุนในแต่ละรอบ

		-- ปรับตำแหน่งวัตถุ
		SetEntityCoords(tree,
			GetEntityCoords(tree).x,
			GetEntityCoords(tree).y,
			currentZ,
			false, false, false, false
		)

		-- ปรับการหมุนวัตถุ
		SetEntityRotation(tree, 0.0, 0.0, currentRotationZ, 2, true)
...
-- เก็บต้นคริสมาส
local function CollectChristmasTree(pointIndex, coords)
	if isCollecting then return end
	isCollecting = true

	local tree = CreateChristmasTree(coords.x, coords.y, coords.z)
	treeObjects[pointIndex] = tree

	-- เล่นแอนิเมชันขุด
	local playerPed = PlayerPedId()
	Wait(500)
	RaiseTreeAnimation(tree, coords.z)


	PlaySoundFrontend("REWARD_COLLECTED", "HUD_REWARD_SOUNDSET", true, 0)
...
CreateThread(function()
    while true do
		...
		if isCollecting then 
			...
			DisableControlAction(0, Keys['B'], false)
...
-- Event Handlers
RegisterNetEvent('christmas:syncCollectedTrees')
AddEventHandler('christmas:syncCollectedTrees', function(syncedTrees)
	collectedTrees = syncedTrees
	isInitialized = true
	print(json.encode(collectedTrees))
	CreateTreeBlips() -- สร้าง blips ใหม่หลัง sync
end)

RegisterNetEvent('christmas:treeCollected')
AddEventHandler('christmas:treeCollected', function(pointIndex)
    -- ตั้งค่าจุดที่เก็บแล้วเป็น true
    collectedTrees[tostring(pointIndex)] = true  -- ใช้ tostring เพื่อให้สอดคล้องกับรูปแบบของ collectedTrees
    
    -- ลบ blip ที่เกี่ยวข้องกับจุดนี้
    if treeBlips[tonumber(pointIndex)] then
        RemoveBlip(treeBlips[tonumber(pointIndex)])
        treeBlips[tonumber(pointIndex)] = nil
    end
end)

function CalculateIdealPosition(playerCoords, objCoords)
    local minDistance = 1.3 -- ระยะห่างขั้นต่ำที่ต้องการ
    ...
    -- คำนวณตำแหน่งที่ควรจะอยู่
    return vec3(
        objCoords.x + (direction.x * minDistance),
        objCoords.y + (direction.y * minDistance),
        objCoords.z
    )
end
...
function Anim(ped, dict, name)
	...
	TaskPlayAnim(ped, dict, name, 1.0, 8.0, -1, 1, 0, false, false, false)
end


local icon1 = false
local icon2 =  { iconDict = "blips", iconName = 'blip_radius_search',color = {r = 255, g = 255, b = 153, a = 250}}  
	...
	while true do
		...
		if isInitialized then
			...
			for i, point in ipairs(Config.TreePoints) do
				...
				-- ตรวจสอบว่า collectedTrees มีค่านี้หรือไม่
				if collectedTrees[treeKey] ~= true then
					local distance = #(playerCoords - vector3(point.x, point.y, point.z))
					if distance < 4 then 
						DrawTexticon(point.x, point.y, point.z-1.4, icon1, icon2)
					end
					if distance < Config.CollectionRadius then
						
						local label = CreateVarString(10, 'LITERAL_STRING', '~o~ ??  ~q~  ')
						PromptSetActiveGroupThisFrame(BordPrompts, label)
						if PromptHasHoldModeCompleted(OpenToggleUI) and not IsPedOnMount(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId()) and not IsKey then
						
							local playerCoords = GetEntityCoords(PlayerPedId())
							local distance = #(playerCoords - vector3(point.x, point.y, point.z))
					
							if distance < 1.3 then
								-- คำนวณตำแหน่งที่ควรถอยไป
								local idealPos = CalculateIdealPosition(playerCoords, vector3(point.x, point.y, point.z))
					
								-- ถอยหลังไปยังตำแหน่งที่เหมาะสม
								TaskGoStraightToCoord(PlayerPedId(),
									idealPos.x, idealPos.y, idealPos.z,
									2.0,                             -- ความเร็ว
									500,                             -- ระยะเวลา (ms)
									GetEntityHeading(PlayerPedId()), -- heading เริ่มต้น
									0.1                              -- stopping range
								)
								Wait(800)                            -- รอให้การเคลื่อนที่เสร็จสิ้น
							end
							
							GetHeadDirection(PlayerPedId())
							SetEntityHeadingLookAt(PlayerPedId(), vector3(point.x, point.y, point.z))
							Wait(500) -- รอให้การหันเสร็จสิ้น
							
							local totalTime = math.random(18, 20)
							local totalKeysRequired = math.random(8, 10)
							local res = true
							if res and not IsKey  then 
								
								TriggerServerEvent('christmas:collectTree', i)
								Anim(playerPed, 'script_mp@last_round@photos', 'pose1_m04')
								CollectChristmasTree(treeKey, vector3(point.x, point.y, point.z))
								IsKey  = true
							end
							Wait(1000)
							IsKey = false
...
function DrawTexticon(x, y, z,icon1,icon2)
    local onScreen,_x,_y=GetScreenCoordFromWorldCoord(x, y, z)
    ...
    if onScreen then
        if icon1 ~= false then  
            ...
            DrawTexture(icon1.iconDict, icon1.iconName,_x, _y-0.25,0.03,  0.05, 0.0, icon1.color.r,icon1.color.g,icon1.color.b,icon1.color.a, true)
        end
        if icon2 ~= false then      
            DrawTexture(icon2.iconDict, icon2.iconName,_x, _y-0.25,0.03,  0.05, 0.0, icon2.color.r,icon2.color.g,icon2.color.b,icon2.color.a, true)
        end
    end
end
...
AddEventHandler('onResourceStop', function(resourceName)
	...
	RemoveAllBlips()
	for _, tree in pairs(treeObjects) do
		if DoesEntityExist(tree) then
			DeleteObject(tree)
		end
	end
